<script lang="ts">
	import type { MetaProps } from './../interfaces/MetaProps';
  export let metadata: MetaProps = {};
</script>

<svelte:head>
  {#if metadata.title}
    <title>{metadata.title} • Tannatee Juchan's space</title>
    <meta name="title" content={metadata.title} />
    <meta property="og:title" content={metadata.title} />
    <meta property="twitter:title" content={metadata.title} />
  {/if}
  
  {#if metadata.description}
    <meta name="description" content={metadata.description} />
    <meta property="og:description" content={metadata.description} />
    <meta property="twitter:description" content={metadata.description} />
  {/if} 
  
  {#if metadata.image}
    <meta property="og:image" content="teelyjc.com/path_to/thumbnail.jpg" />
    <meta
      property="twitter:image"
      content="https://teelyjc.com/path-to/thumbnail.jpg"
    />
  {/if} 
  
  {#if metadata.imageAlt}
    <meta property="og:image:alt" content={metadata.imageAlt} />
    <meta property="twitter:image:alt" content={metadata.imageAlt} />
  {/if}

  {#if metadata.url}
    <meta property="og:url" content={metadata.url} />
    <meta property="twitter:url" content={metadata.url} />
  {/if}

 	{#if metadata.type}
	  <meta property="og:type" content={metadata.type} />
  {/if}

  <meta property="twitter:card" content={metadata.twitterCard || "summary_large_image"} />
</svelte:head>